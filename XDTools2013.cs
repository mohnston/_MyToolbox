...
namespace CADHelp
{
    public class XDTools
    {
        ...
        public static Hashtable GetMyXDValues(
            ObjectId myOID,
            string appName)
        {
            ...
            {
                resHT = GetMyXDValues(db, trans, myOID, appName);
                trans.Commit();
            } 
            ...
            if (rb != null) // found xdata for the appName
            {
                foreach (TypedValue tv in rb)
                {
                    if (tv.TypeCode == 1000)
                    {
                        ...
                        fld = ParseXDFieldName(tval);
                        val = ParseXDValue(tval);
                        if (!resHT.ContainsKey(fld))
                        {
                            resHT.Add(fld, val);
                        }
                    }
        ...
        public static void SetMyXDValues(
            ObjectId myOID,
            string appName,
            Hashtable htXData,
            bool ReplaceIfExists)
        {
            ...
            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                SetMyXDValues(db, trans, myOID, appName, htXData, ReplaceIfExists);
                trans.Commit();
            }
            ...
            bool ReplaceIfExists)
        {
            ...
            using (DocumentLock dlock = doc.LockDocument())
            {
                ...
                if ((rb == null) || (rbAll == null)) // no xdata at all for this object OR no xdata for the appName
                {
                    tvList.Add(new TypedValue(1001, appName));
                    //rb = new ResultBuffer(
                    //    new TypedValue(1001, appName),
                    //    new TypedValue(1000, fieldName + "::" + fieldValue));
                }
                ...
                {
                    tvList = new TypedValueList(rb.AsArray());
                }
                //else // rb is not null and reg app is already in drawing
                //{
                    // Create typedvalue list so items can be added, removed and replaced

                foreach (DictionaryEntry de in htXData)
                {
                    string fieldName = de.Key.ToString();
                    string fieldValue = de.Value.ToString();
                    if (ReplaceIfExists == false)
                    {
                        tvList.Add(1000, fieldName + "::" + fieldValue);
                    }
